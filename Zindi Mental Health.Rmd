...
d_tr2 <- select(d_tr,-c(11))
d_ts2 <- select(d_ts,-c(11))
...
d_ts2$age <- as.numeric(d_ts2$age)

# parse dates accordingly
d_ts2$survey_date <- lubridate::parse_date_time(d_ts2$survey_date,"dmy")
d_tr3$survey_date <- lubridate::parse_date_time(d_tr3$survey_date,"dmy")
...
mice_plot <- aggr(d_tr3,col = c('navyblue','yellow'),
...
imputedDT <- mice(d_tr3, m = 5, maxit = 50, method = "pmm", seed = 121,threshold = 1)
# summary(imputedDT)
...
d_ts4 <- as.h2o(d_ts2, destination_frame = "d_ts4")
```
